...
fn allowance_profile(path: &PathBuf) -> Profile {
    Profile::new(vec![Operation::FileReadAll(PathPattern::Literal(path.clone()))]).unwrap()
}

fn prohibition_profile() -> Profile {
    Profile::new(vec![Operation::FileReadAll(PathPattern::Subpath(PathBuf::from("/bogus")))])
        .unwrap()
}
