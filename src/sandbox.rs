...
///
/// A new sandbox can be created with `Sandbox::new()`, which all
/// platform-specific sandboxes implement.
...
pub trait ChildSandboxMethods {
    /// Activates the restrictions in this child process from here on out. Be
...
impl Command {
    /// Constructs a new `Command` for launching the executable at path
    /// `module_path` with no arguments and no environment by default.
    ...
    where
        T: AsRef<OsStr>,
    {
        Command { module_path: cstring(module_path), args: Vec::new(), env: HashMap::new() }
    }
