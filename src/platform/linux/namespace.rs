...
impl ChrootJail {
    ...
    fn new(profile: &Profile) -> Result<ChrootJail, c_int> {
        ...
        let mut prefix: Vec<u8> = prefix.as_bytes_with_nul().to_vec();
        unsafe {
        ...
        let dest = CString::new(jail.directory.as_os_str().to_str().unwrap().as_bytes()).unwrap();
    ...
    fn enter(&self) -> Result<(), c_int> {
        let directory =
            CString::new(self.directory.as_os_str().to_str().unwrap().as_bytes()).unwrap();
        let result = unsafe { libc::chroot(directory.as_ptr()) };
        if result != 0 {
            return Err(result);
        }
    ...
    fn bind_mount(&self, source_path: &Path) -> Result<(), c_int> {
        ...
        let mut components: Vec<OsString> = source_path
            .components()
            .skip(1)
        ...
        let source_path =
            CString::new(source_path.as_os_str().to_str().unwrap().as_bytes()).unwrap();
        let destination_path =
            CString::new(destination_path.as_os_str().to_str().unwrap().as_bytes()).unwrap();
...
/// Removes fake-superuser capabilities. This removes our ability to mess with
/// the filesystem view we've set up.
fn drop_capabilities() -> Result<(), c_int> {
    let capability_data: Vec<_> =
        iter::repeat(__user_cap_data_struct { effective: 0, permitted: 0, inheritable: 0 })
...
pub fn start(profile: &Profile, command: &mut Command) -> io::Result<Process> {
    // Store our root namespace UID and GID because they're going to change once we
    ...
    // Additionally, if we aren't allowing network operations, create a network
    // namespace.
    ...
    unsafe {
        ...
        // Set this `prctl` flag so that we can wait on our grandchild. (Otherwise it'll
        // be reparented to init.)
        ...
        if libc::fork() == 0 {
            ...
            // Set up our user and PID namespaces. The PID namespace won't actually come
            // into effect until the next fork(), because PIDs are immutable.
        ...
        // Grandparent execution continues here. First, close the writing end of the
        // pipe.
