...
pub fn activate() -> Result<(), libc::c_int> {
    ...
    let result = unsafe { libc::setrlimit(libc::RLIMIT_FSIZE, &rlimit) };
    if result != 0 {
        return Err(result);
    }

    // Set a restrictive `umask` so that even if files happened to get written it'd
    // be hard to do anything with them.
    unsafe {
        libc::umask(0);
    }
    ...
    // Enter a new session group. (This can fail with -EPERM if we're already the
    // session leader, which is OK.)
    unsafe {
        if libc::setsid() < 0 {
    ...
    let result = unsafe { libc::clearenv() };
    if result == 0 {
        Ok(())
    } else {
        Err(result)
